...
bool PeacockHash::initialFromFile(char *InputFileName, float ht_times, unsigned int stage, unsigned int MAX_READ) {
    //initial Hash Table
    int lines = 0;
    //get number of lines of inputfile
    hashentryP *pHashentries = readHashEntriesPointer(lines, InputFileName);

    lines = min((int) MAX_READ, lines);

    unsigned int r = 8; //default ratio is 10
...
bool PeacockHash::init(int stage, int scaling_rate, int total_elements, int bf_length) {

    unsigned int len_last = (unsigned int) (1.0 * total_elements * (scaling_rate - 1) /
                                            (pow(scaling_rate, stage) - 1.0));
    unsigned int len_main = len_last * (unsigned int) pow(scaling_rate, stage - 1);
    ...
    if (len_main % (int) pow(scaling_rate, stage - 1) != 0) {
        throw(" len_main must be equal to a positive integer times r^(stage-1) ");
    }

    // calculate length of hash tables
    hashlen = new unsigned int[stage];
    double size_rate[20], tot_rate = 1.0;
    size_rate[0] = 1;

    for (int k = 1; k < stage; ++k) {
        size_rate[k] = size_rate[k - 1] / scaling_rate;
        tot_rate += size_rate[k];
    }
    this->totsize = 0;
    for (int k = 0; k < stage; ++k) {
        hashlen[k] = (unsigned) (total_elements * size_rate[k] / tot_rate);
        this->totsize += hashlen[k];
    }
    len_main = hashlen[0];
    len_last = hashlen[stage - 1];
    ...
    BloomFilter = new StandardBF[stage - 1];
    for (int i = 1; i < stage; i++) {
        BloomFilter[i - 1].initial((unsigned) (bf_length / (stage - 1)),
                                   BF_HASH_FUNC_NUM); //the second parameter is the number of hash function
        //used in bloom filter
        printf("%d %d\n", hashlen[i], (unsigned) (bf_length * size_rate[i] / (tot_rate - size_rate[0])));
    }
    ...
    ratiofactor = (unsigned) scaling_rate;
    NumStages = (unsigned) stage;
        ...
        for (int i = 1; i < NumStages; i++) {
            if (mask[i] == true) {
                ...
                if (memcmp(subhash[pos].key, key, len) == 0) {
                    ...
                    if (memAccCount < MAX_SEARCH_MEM_ACC) {
                        searchMemAccCounter[memAccCount]++;
                    }
                    searchMAsucc += succ;
                    return true;
                }
...
bool PeacockHash::clearSearchStat() {
    searchCount = 0;
    searchCountTot = 0;
    searchMAtot = 0;
    searchMAsucc = 0;
    ...
    searchMAtotP = 0;
    NumCollisions = 0;
    for (int i = 0; i < MAX_SEARCH_MEM_ACC; ++i) {
        searchMemAccCounter[i] = 0;
    }
    for (int i = 1; i < NumStages; ++i) {
        BloomFilter[i - 1].QuerymemAccNum = 0;
        BloomFilter[i - 1].QuerymemAccNumSucc = 0;
        BloomFilter[i - 1].QuerymemAccNumFail = 0;
        BloomFilter[i - 1].InsertmemAccNum = 0;
    }
    return true;
}
...
void PeacockHash::report(char *outputfile) {
    ...
    fprintf((FILE *) file, "LoadingFactor/NumCollisions: %f\t%d\n", loadingfactorT,
            NumCollisions); // yt: we need collision rate.
    for (int i = 0; i < NumStages; i++) {
...
void PeacockHash::reportInline(FILE *fp,bool failed) {
    if(failed){
        uint OnChipFail = 0;
        for (int i = 1; i < NumStages; ++i) {
            OnChipFail += BloomFilter[i - 1].QuerymemAccNumFail;
        }
        fprintf(fp, "%.5lf\t\t", (0.0 + searchMAfail)/ MEMORY_ACC_UNIT);
        fprintf(fp, "%.5lf\t\t", (0.0 + OnChipFail) / MEMORY_ACC_UNIT);
        fprintf(fp, "\n");
        return;
    }

    fprintf(fp, "%.5lf\t\t", loadingfactorT);

    fprintf(fp, "%.5lf\t\t", (0.0 + NumCollisions) / MEMORY_ACC_UNIT);

    fprintf(fp, "%.5lf\t\t", (0.0 + insertMAtot) / MEMORY_ACC_UNIT);

    uint OnChipMem = 0;
    for (int i = 1; i < NumStages; ++i) {
        OnChipMem += BloomFilter[i - 1].InsertmemAccNum;
    }
    fprintf(fp, "%.5lf\t\t", (0.0 + OnChipMem) / MEMORY_ACC_UNIT);

    uint OnChipSucc = 0;
    for (int i = 1; i < NumStages; ++i) {
        OnChipSucc += BloomFilter[i - 1].QuerymemAccNumSucc;
    }
   
    fprintf(fp, "%.5lf\t\t", (0.0 + OnChipSucc) / MEMORY_ACC_UNIT);
   
    fprintf(fp, "%.5lf\t\t", (0.0 + searchMAsucc) / MEMORY_ACC_UNIT);
    printf("searchMAsucc = %d\n",searchMAsucc);
    // return
    fprintf(fp, "\n");
}
