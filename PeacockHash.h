...
public:
	...
	*/
	bool initialFromFile(char *InputFileName, float ht_times , unsigned int stage, unsigned int MAX_READ);

	bool init(int stage, int scaling_rate, int total_elements, int bf_length);
	...
	*/
	...
	void reportInline(FILE * fp,bool failed);
	bool reset();
	bool clearSearchStat();
	bool test(char *File);
...
public:
	...
	unsigned int searchCountTot;
	unsigned int insertCountTot;
	...
	// record count for diff mem acc times
	int searchMemAccCounter[MAX_SEARCH_MEM_ACC];
	...
	unsigned int searchMAsucc;
	unsigned int searchMAfail;
	unsigned int insertMAtot;
	unsigned int removeMAtot;

	//
	unsigned int searchMAtotP;
